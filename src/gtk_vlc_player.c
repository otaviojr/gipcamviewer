...
static void vlc_media_player_opening(const struct libvlc_event_t *event, void *user_data);
static void vlc_media_player_playing(const struct libvlc_event_t *event, void *user_data);
static void vlc_media_player_stopped(const struct libvlc_event_t *event, void *user_data);
static void vlc_media_player_state_changed(const struct libvlc_event_t *event, void *user_data);
static void vlc_media_player_error(const struct libvlc_event_t *event, void *user_data);
...
create_vlc_instance(void)
{
	...
	//vlc_argv[vlc_argc++] = g_strdup("--sout-rtp-caching=1000");
	//vlc_argv[vlc_argc++] = g_strdup("--avcodec-fast");
	//vlc_argv[vlc_argc++] = g_strdup("--avcodec-skiploopfilter=4");
	//vlc_argv[vlc_argc++] = g_strdup("--avcodec-dr");
	//vlc_argv[vlc_argc++] = g_strdup("--rtsp-frame-buffer-size=20971520");
	//vlc_argv[vlc_argc++] = g_strdup("--clock-jitter=0");
	//vlc_argv[vlc_argc++] = g_strdup("--clock-synchr=0");
	vlc_argv[vlc_argc++] = g_strdup("--no-plugins-cache");
...
gtk_vlc_player_init(GtkVlcPlayer *klass)
{
  ...
  gtk_style_context_add_class(context,"vlc-player-drawing");
	...
	libvlc_event_attach(evman, libvlc_MediaPlayerOpening,
			    vlc_media_player_opening, klass);
	libvlc_event_attach(evman, libvlc_MediaPlayerPlaying,
			    vlc_media_player_playing, klass);
	libvlc_event_attach(evman, libvlc_MediaPlayerStopped,
			    vlc_media_player_stopped, klass);
	libvlc_event_attach(evman, libvlc_MediaStateChanged,
			    vlc_media_player_state_changed, klass);
	libvlc_event_attach(evman, libvlc_MediaPlayerEncounteredError,
			    vlc_media_player_error, klass);
...
static void
vlc_media_player_opening(const struct libvlc_event_t *event, void *user_data)
{
	assert(event->type == libvlc_MediaPlayerOpening);
	printf("vlc_media_player_opening\n");
}
...
static void
vlc_media_player_stopped(const struct libvlc_event_t *event, void *user_data)
{
	assert(event->type == libvlc_MediaPlayerStopped);
	printf("vlc_media_player_stopped\n");
}

static void
vlc_media_player_state_changed(const struct libvlc_event_t *event, void *user_data)
{
	assert(event->type == libvlc_MediaStateChanged);
	printf("vlc_media_player_state_changed\n");
}

static void
vlc_media_player_error(const struct libvlc_event_t *event, void *user_data)
{
	assert(event->type == libvlc_MediaPlayerEncounteredError);
	printf("vlc_media_player_error\n");
}
...
gtk_vlc_player_set_time_adjustment(GtkVlcPlayer *player, GtkAdjustment *adj)
{
	...
	g_object_unref(player->priv->time_adjustment);
	player->priv->time_adjustment = GTK_ADJUSTMENT(adj);
...
gtk_vlc_player_set_volume_adjustment(GtkVlcPlayer *player, GtkAdjustment *adj)
{
	...
	g_object_unref(player->priv->volume_adjustment);
	player->priv->volume_adjustment = GTK_ADJUSTMENT(adj);
